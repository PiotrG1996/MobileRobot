...
Mobile robots can be divided according to the power transmission system of a robot. The popularization of automation systems has caused a huge interest in designing and inventing new things by amateur constructors. These kinds of robots are not only used in competitions or shows but also in real industry and production processes.
One of the most important features of mobile robots is their high manoeuvrability and freedom of movement. An interesting solution is taking advantage of special omnidirectional wheels also called rollers. The wheels allow moving in every direction without rotation. It means that it is possible to change the direction of movement (orientation) in place. This solution makes it possible to apply in different technology fields like internal transport, logistics, research, monitoring and among others.
...
Due to various ways of movement, mobile robots can be grouped into categories:
...
In this scenario, you don't need to use Arduino in order to control electrical motors. An external DC motor controller can be used directly on Raspberry Pi. This external hat can be found [here](https://wiki.dfrobot.com/DC_Motor_Driver_HAT_SKU_DFR0592).


### **Custom Arduino PCB shield**

<details><summary>PCB Layout</summary>
</details>

### **Model 3D**

<details><summary>3D Printed Construction</summary>
  
These 3D top and bottom covers were designed in SolidWorks and manufactured using a 3D printer. The results are presented below:
...
The idea of this project is to control a mobile robot via local wifi using only basic web technologies. Therefore the usage of a well-known HTTP protocol is reasonable and will be implemented. The biggest advantage of using web applications is that we are not restricted to any operating system. This application will be running in a web browser. Client-Server architecture allows for to separation of Frontend and Backend code, in this case, the server contains the whole functionality and the client renders the user interface.
...
![source](https://yck-1254263422.cos.ap-shanghai.myqcloud.com/blog/2019-06-01-042653.png)
